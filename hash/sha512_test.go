...
func TestSHA512HashPath(t *testing.T) {
	...
	hash, err = SHA512PathHex(foo.Name())
	require.NoError(t, err, "Error hashing text to using %s", SHA512Hash)
	assert.NotEmpty(t, hash)

	hash, err = SHA512PathBase64StdEnc(dir)
	require.NoError(t, err, "Error hashing text to using %s", SHA512Hash)
	assert.NotEmpty(t, hash)

	hash, err = SHA512PathBase64StdEnc(foo.Name())
	require.NoError(t, err, "Error hashing text to using %s", SHA512Hash)
	assert.NotEmpty(t, hash)

	hash, err = SHA512PathBase64URLEnc(dir)
	require.NoError(t, err, "Error hashing text to using %s", SHA512Hash)
	assert.NotEmpty(t, hash)

	hash, err = SHA512PathBase64URLEnc(foo.Name())
	require.NoError(t, err, "Error hashing text to using %s", SHA512Hash)
	assert.NotEmpty(t, hash)

	hash, err = SHA512PathBase64RawURLEnc(dir)
	require.NoError(t, err, "Error hashing text to using %s", SHA512Hash)
	assert.NotEmpty(t, hash)

	hash, err = SHA512PathBase64RawURLEnc(foo.Name())
	require.NoError(t, err, "Error hashing text to using %s", SHA512Hash)
	assert.NotEmpty(t, hash)

	hash, err = SHA512PathBase64RawStdEnc(dir)
	require.NoError(t, err, "Error hashing text to using %s", SHA512Hash)
	assert.NotEmpty(t, hash)

	hash, err = SHA512PathBase64RawStdEnc(foo.Name())
	require.NoError(t, err, "Error hashing text to using %s", SHA512Hash)
	assert.NotEmpty(t, hash)
}
